...
class AuthenticatedGenerator < Rails::Generator::NamedBase
  ...
  def manifest
    recorded_session = record do |m|
      ...
      m.directory File.join('  /models', class_path)
      m.directory File.join('  /controllers', controller_class_path)
      m.directory File.join('  /controllers', model_controller_class_path)
      m.directory File.join('  /helpers', controller_class_path)
      m.directory File.join('  /views', controller_class_path, controller_file_name)
      m.directory File.join('  /views', class_path, "#{file_name}_mailer") if options[:include_activation]

      m.directory File.join('  /controllers', model_controller_class_path)
      m.directory File.join('  /helpers', model_controller_class_path)
      m.directory File.join('  /views', model_controller_class_path, model_controller_file_name)
      ...
      if options[:include_activation]
        %w( mailer observer ).each do |model_type|
          m.template "#{model_type}.rb", File.join('  /models',
                                               class_path,
      ...
      m.template 'login.html.erb',  File.join('  /views', controller_class_path, controller_file_name, "new.html.erb")
      m.template 'signup.html.erb', File.join('  /views', model_controller_class_path, model_controller_file_name, "new.html.erb")
      m.template '_model_partial.html.erb', File.join('  /views', model_controller_class_path, model_controller_file_name, "_#{file_name}_bar.html.erb")

      if options[:include_activation]
        # Mailer templates
        %w( activation signup_notification ).each do |action|
          m.template "#{action}.erb",
                     File.join('  /views', "#{file_name}_mailer", "#{action}.erb")
        end
      end
